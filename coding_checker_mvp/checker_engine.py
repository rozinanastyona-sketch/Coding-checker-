...
def write_scores_sheet(wb, scores_rows: List[Dict[str, Any]]) -> None:
    ...
    zero_fill = PatternFill("solid", fgColor="FF9999")
    for r, row in enumerate(scores_rows, start=2):
        for col, h in enumerate(headers, start=1):
            cell = ws.cell(row=r, column=col, value=row.get(h))
            if h not in ("Number", "Time_Relative_hms", "Utterance") and row.get(h) == 0:
                cell.fill = zero_fill
...
def write_annotated_excel(
    ...
    # Map each original row index to the utterance it belongs to, and add an
    # "Utterance #" column so that's visible on every row, not just flagged ones.
    ...
    utt_col = header_map.get("Utterance #")
    if student_utts and utt_col is None:
        utt_col = ws.max_column + 1
        ws.cell(row=1, column=utt_col).value = "Utterance #"
        ws.cell(row=1, column=utt_col).font = Font(bold=True)
        for original_index, utt in row_to_utt.items():
            excel_row = original_index + 2
            ws.cell(row=excel_row, column=utt_col).value = utt.uid
    ...
    for issue in insert_issues:
        ...
        if utt_col is not None and issue.utterance_id is not None:
            ws.cell(row=insert_at, column=utt_col).value = issue.utterance_id
